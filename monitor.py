...
def monitoruna():
    ...
    for i in range(num_nodes):
        unanode.append(match[2*i])
        unacore.append(len(match[2*i+1]))
    return unanode, unacore

def monitordebug(cpu=4,gpu=1,memory=8192):
    node,core = monitoruna()
    ...
    for i in range(num_nodes):
        acpu = int(match[7*i+3])
        amemory = int(match[7*i+5])
        agpu = int(match[7*i+1])
        if (match[7*i] in node):
            idx = node.index(match[7*i])
            agpu = agpu - int(core[idx])
        if (acpu>=int(cpu)) and (agpu>=int(gpu)) and (amemory>=int(memory)):
            flag = 1
            text += 'Node ' + str(match[7*i+0])+ ', CPU Left ' + str(acpu) + ', GPU Left ' + str(agpu) + ', Memory Left ' + str(amemory) + '\n'
